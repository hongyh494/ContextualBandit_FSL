...
def apply_K_best(ax, bandit_10_arm, max_J=10, K=3, num_episodes=100, num_steps=20000):
    """K Best策略"""
    L = max_J - K  # Calculate L based on max_J and K

    def strategy(state):
        """Strategy based on losing streak state S_t."""
        if 0 <= state < K:
            return bandit_10_arm.K-1  # Choose arm A (represented as 0)
        else:
            return 0  # Choose arm B (represented as 1)
    ...
    for i in range(10):  # 显示10个进度条
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 10)):  # 每个进度条里的序列数目
                episode_return = 0
                state = bandit_10_arm.reset()  # bandit : 重置老虎机状态
                step_i = 0  # 步数计数器 每条序列采样的长度
                while step_i < num_steps:
                    # 根据策略选择动作手臂
                    action = strategy(state)
                    reward, next_state = bandit_10_arm.step(action)  # bandit : 摇臂，得到t奖励和t+1状态
    ...
    plot_accum_reward(ax, bandit_10_arm, num_steps, return_list, method='K Best', window_size=10)

    # 打印策略
    print("K Best Strategy:")
    for state in range(max_J):
        print(f"State {state}: Choose arm {strategy(state)}")
...
if __name__ == "__main__":
    ...
    max_J=6  # 设定最大J值，状态就有0，..., J-1这几种
    J_distribution = ('static', 6)  # 离散分布
    # J_distribution = ('discrete', [2, 3, 4, 5, 6], [0.2]*5)  # 离散分布

    # fig, ax = plt.subplots(2, 2, figsize=(12, 6))
    # ax = ax.flatten()
    # fig, ax = plt.subplots(1, 1, figsize=(12, 8))  # 不同的K再换
    fig, ax = plt.subplots(1, 1, figsize=(12, 8))  # 不同的K再换
    ...
    apply_sarsa(ax, seed1, bandit_bernoulli, gamma=0.9,
                 num_episodes=num_episodes, num_steps=num_steps, max_J=max_J)

    apply_K_best(ax, bandit_bernoulli, max_J=max_J, K=3, num_episodes=num_episodes, num_steps=num_steps)
    
    # 只有连输状态的dqn
    apply_dqn(ax, seed1, bandit_bernoulli, state_dim=1, hidden_dim=32, action_dim=K, 
    ...
    ax.axhline(y=num_steps, linestyle='--', color='gray', label='Compensation Baseline')

    # [state, action] 动作对的dqn, 输出一个标量Q(s,a)
    plt.tight_layout()
    plt.legend(fontsize=20, loc='upper right')
    plt.savefig('best_strategy_plot.png', dpi=600, bbox_inches='tight')
    plt.show()
